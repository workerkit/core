...
const directoryOverview: ToolDescriptor = {
  ...
  description:
    "The directory's vocabulary and scale in one call — call this FIRST when you need valid kits_search filter values or a map of what WorkerKit covers. Returns: kitCount (published kits right now); categories grouped as jobFamilies/roles/industries ({slug, name} — slugs feed the category/industry filters); apps ({code, name, categoryCode?, installable} — codes feed the app filter; installable:false means kits can show the app but an install cannot carry it); appCategories ({code, name, members:[{code, name, appCode}]} — codes feed the appCategory filter; members are what an installer picks between, e.g. Slack vs Teams); models ({model, normalized, kitCount} — pass normalized or any substring as the model filter); sortOptions; triggerModes. Vocabulary only — it lists no kits: for 'what is popular', use kits_search with the default downloads sort. Kit pages for humans follow https://workerkit.ai/kit/{slug}." +
    DIRECTORY_NOTE,
  auth: "anonymous",
  method: "get",
  schema: {},
...
const getKit: ToolDescriptor = {
  ...
  title: "Get Kit",
  description:
    "One kit's full public dossier — everything a human (or an agent advising one) can evaluate before installing. Key fields: description + appDescriptions (author's per-app/per-tool notes); instructionContent — the worker's job instruction VERBATIM, the core of what you are evaluating (empty string when isProtected while instructionLength still reports the real size; startCommand/endCommand/whenToUse are null on a protected kit, deliberately indistinguishable from 'none'); permissionsManifest — the EXACT data access an install grants (per-app operations/fields/scopes; numeric value is authoritative, names/keys are display) — summarize THIS when asked what the kit can touch; requiredInputs + memorySetup — the install form: fields the kit needs and questions whose answers become the worker's memory; memoryProfile ('contextual' = the worker is shown its own recent runs) and selfFactsEnabled (the worker saves durable facts on its own); schedules/triggers/usageWindows; skillResources — reference docs the worker fetches on demand: key/kind/name always, plus description/contentLength on a non-protected kit (a protected kit lists identity only), body via includeResourceBodies; hasExternalMcpShells — TRUE means installing creates an MCP gateway in the installer's account pointing at the publisher's own upstream URL (permissionsManifest.mcpServers.shells) — surface this when recommending; scan — the security scan's result for the text being served: outcome 'cleared' or 'underReview' (BOTH are live, installable listings; 'underReview' means a check raised something a reviewer has not settled, never that the kit is unsafe), scannedAtUtc (null = published before the scanner existed, which is most of the catalog), checks — the check families that actually ran on THIS kit, so 'cleared' with only ['static'] is a weaker statement than 'cleared' with all three; and recommendation ('safe' | 'caution' | 'doNotInstall') — the SCANNER's own install advice about the content, which is independent of outcome (our moderation state) and may legitimately disagree with it, so attribute it to SkillSpector rather than to WorkerKit when you repeat it, and read 'caution' as 'worth a look', not as danger (the scanner also emits it when its own analysis was incomplete); scan is absent for a kit with nothing to report, and it never carries the findings themselves; modelScores (with notes), downloadCount/starCount, and pageUrl — the install page to hand to the human. 404 = no published kit with that slug." +
    DIRECTORY_NOTE,
  auth: "anonymous",
  method: "get",
  schema: {
